...
echo.
echo   [1/5] Обновляю %PROPS%  ->  ^<Version^>%NUMERIC%^</Version^>
powershell -NoProfile -Command "$p='%PROPS%'; $c=Get-Content $p -Raw; $c=[regex]::Replace($c,'<Version>.*?</Version>','<Version>%NUMERIC%</Version>'); Set-Content $p $c -Encoding UTF8 -NoNewline"
...
echo     - SimryxSetup.exe            установщик (появится после Части 2)
echo ==================================================
echo.
pause
exit /b 0
