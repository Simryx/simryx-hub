...
public sealed class InstallService
{
		...
		InstallOptions o, IProgress<ProgressReport> p, CancellationToken ct = default)
	{
		...
		var tmpZip = Path.Combine(Path.GetTempPath(), $"SimryxHub{Guid.NewGuid():N}.zip");
		...
		ExtractArchive(tmpZip, installDir);
		...
		IProgress<ProgressReport> p, CancellationToken ct = default)
	{
		...
		// Свежий SimryxSetup.exe приходит ВНУТРИ пакета обновления и уже распакован —
		// оставляем именно его (правильная тема окна, актуальный деинсталлятор).
		// Если в пакете его не оказалось (старый формат) — кладём текущий процесс,
		// чтобы не потерять деинсталлятор.
		TryApplyPendingInstaller(dir);
		var uninstExe = Path.Combine(dir, "SimryxSetup.exe");
		if (!File.Exists(uninstExe))
		{
			var selfExe = Environment.ProcessPath!;
			try { File.Copy(selfExe, uninstExe, true); }
			catch { uninstExe = selfExe; }
		}
	...
	/// <summary>
	/// Безопасная распаковка ZIP-пакета. Каждый файл распаковывается отдельно,
	/// чтобы единичная блокировка (например, выполняющийся SimryxSetup.exe)
	/// не срывала всё обновление и не оставляла установку «пустой».
	/// Если целевой файл совпадает с выполняющимся процессом (заблокирован),
	/// пишем рядом «*.new» — он применится при следующем запуске установщика.
	/// </summary>
	private static void ExtractArchive(string zipPath, string destDir)
	{
		...
		foreach (var entry in archive.Entries)
		{
			...
			// Защита от Zip Slip: не выходим за пределы папки установки.
			if (!string.Equals(target, destFull, StringComparison.OrdinalIgnoreCase) &&
				!target.StartsWith(destFull + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
				continue;
			...
			// Файл занят текущим процессом — отложенная подмена.
			if (!string.IsNullOrEmpty(self) &&
				string.Equals(target, self, StringComparison.OrdinalIgnoreCase))
			{
				TryExtract(entry, target + ".new");
				continue;
			}

			if (!TryExtract(entry, target))
				TryExtract(entry, target + ".new"); // запасной путь при блокировке
		}
	}

	private static bool TryExtract(ZipArchiveEntry entry, string target)
	{
		try { entry.ExtractToFile(target, overwrite: true); return true; }
		catch { return false; }
	}

	// Применяет отложенную замену установщика (SimryxSetup.exe.new -> SimryxSetup.exe),
	// если основной файл сейчас не заблокирован. В обычном сценарии (запуск из
	// временной копии) «*.new» не создаётся и метод ничего не делает.
	private static void TryApplyPendingInstaller(string dir)
	{
		try
		{
			var real = Path.Combine(dir, "SimryxSetup.exe");
			var pending = real + ".new";
			if (!File.Exists(pending)) return;
			File.Copy(pending, real, true);
			File.Delete(pending);
		}
		catch { /* применим при следующем обновлении */ }
	}
