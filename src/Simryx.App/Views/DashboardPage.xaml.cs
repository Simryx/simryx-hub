...
public sealed partial class DashboardPage : Page
{
    ...
    private void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        // Определение игры — нужен только факт запуска/остановки (идемпотентно).
    ...
    // Старт/стоп игры приходят из фонового потока — маршалим в UI.
    private void OnGameStarted(SimGame game)
        => DispatcherQueue.TryEnqueue(() => { if (_loaded) UpdateGameStatus(); });

    private void OnGameStopped(SimGame game)
        => DispatcherQueue.TryEnqueue(() => { if (_loaded) UpdateGameStatus(); });
    ...
    // Плитка «Игра»: «Не запущена» либо просто название игры (без статуса сессии).
    private void UpdateGameStatus()
    {
        var en = IsEnglish;
        var game = _detection?.CurrentGame;
        StatusGameValue.Text = game is not null
            ? game.Name
            : (en ? "Not running" : "Не запущена");
    }
