...
/// могла показать баннер («Доступно обновление» или «Можно вернуться на стабильную»).
/// </summary>
public static class UpdateCoordinator
{
	...
	/// <summary>Последний результат, в котором есть что установить (если был).</summary>
	public static UpdateCheckResult? LastAvailable { get; private set; }

	/// <summary>Срабатывает, когда тихая проверка нашла обновление или возможность отката.</summary>
	...
	public static async Task RunStartupCheckAsync(CancellationToken ct = default)
	{
		...
		try
		{
			...
			// Тихо показываем баннер, когда есть что установить: обычное обновление
			// ИЛИ возможность вернуться на стабильную версию (откат с беты).
			// CanInstall покрывает оба статуса (UpdateAvailable и RollbackAvailable).
	...
	/// проверку, а устаревший кэш очищается. Вызывается, например, при смене
	/// канала обновлений в «Параметрах».
	/// </summary>
